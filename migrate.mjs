...
async function uploadImage(filename) {
    ...
    const { data, error } = await supabase.storage.from('product_images').upload(`legacy/${filename}`, fileBuf, {
        upsert: true,
...
async function run() {
    ...
    for (const p of products) {
        ...
        if (pErr) {
            console.error(`Error inserting product ${p.slug}:`, pErr);
            continue;
        }
        ...
        for (let i = 0; i < p.variants.length; i++) {
            ...
            if (vErr) {
                console.error(`Error inserting variant ${v.label} for ${p.slug}:`, vErr);
            }
        }
        migratedCount++;
    }
