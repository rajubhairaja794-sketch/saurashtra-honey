...
//#region node_modules/.nitro/vite/services/ssr/assets/newsletter.functions-DbL4tsBS.js
...
var subscribeNewsletter = createServerFn({ method: "POST" }).inputValidator((input) => ({
	...
	const { data: existing } = await supabaseAdmin.from("newsletter_subscribers").select("id, confirmed_at, unsubscribed_at, confirm_token, unsubscribe_token").eq("email", data.email).maybeSingle();
	if (existing?.confirmed_at && !existing.unsubscribed_at) return {
		ok: true,
		already: true
	};
	if (existing) await supabaseAdmin.from("newsletter_subscribers").update({
		source: data.source,
		confirm_token: existing.confirm_token ?? confirm,
		unsubscribe_token: existing.unsubscribe_token ?? unsub,
		unsubscribed_at: null
	}).eq("id", existing.id);
	else await supabaseAdmin.from("newsletter_subscribers").insert({
		email: data.email,
		source: data.source,
...
var confirmNewsletter = createServerFn({ method: "POST" }).inputValidator((input) => ({ token: String(input.token).slice(0, 128) })).handler(confirmNewsletter_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: row } = await supabaseAdmin.from("newsletter_subscribers").select("id, confirmed_at").eq("confirm_token", data.token).maybeSingle();
	if (!row) throw new Error("Invalid or expired link");
	if (!row.confirmed_at) await supabaseAdmin.from("newsletter_subscribers").update({ confirmed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", row.id);
	return { ok: true };
});
...
var unsubscribeNewsletter = createServerFn({ method: "POST" }).inputValidator((input) => ({ token: String(input.token).slice(0, 128) })).handler(unsubscribeNewsletter_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { error } = await supabaseAdmin.from("newsletter_subscribers").update({ unsubscribed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("unsubscribe_token", data.token);
