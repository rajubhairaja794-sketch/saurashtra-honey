...
//#region node_modules/.nitro/vite/services/ssr/assets/admin-catalog.functions-DK9h6I3a.js
...
var upsertProduct = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => productSchema.parse(d)).handler(upsertProduct_createServerFn_handler, async ({ data, context }) => {
	...
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { id, ...rest } = data;
	if (rest.images && Array.isArray(rest.images) && rest.images.length > 0) rest.image_url = rest.images[0];
	...
	if (id) {
		const { data: updated, error } = await supabaseAdmin.from("products").update(rest).eq("id", id).select("id").single();
		if (error) throw new Error(error.message);
		return {
			ok: true,
			id: updated?.id || id
...
var listProductVariants = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ product_id: stringType().uuid() }).parse(d)).handler(listProductVariants_createServerFn_handler, async ({ data, context }) => {
	await assertAdmin(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: rows, error } = await supabaseAdmin.from("product_variants").select("*").eq("product_id", data.product_id).order("sort_order", { ascending: true });
	if (error) {
...
var saveProductVariants = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { data: existing } = await supabaseAdmin.from("product_variants").select("id").eq("product_id", product_id);
	...
	await supabaseAdmin.from("product_variants").update({ is_default: false }).eq("product_id", product_id);
	const results = [];
	for (const v of normalized) if (v.id) {
		const { id, ...rest } = v;
		const { data: updated, error: updErr } = await supabaseAdmin.from("product_variants").update(rest).eq("id", id).select("*").single();
		if (updErr) throw new Error(updErr.message);
		results.push(updated);
	} else {
		const { data: inserted, error: insErr } = await supabaseAdmin.from("product_variants").insert(v).select("*").single();
...
var listAdminSlides = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(listAdminSlides_createServerFn_handler, async ({ context }) => {
	await assertAdmin(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data, error } = await supabaseAdmin.from("hero_slides").select("*").order("page", { ascending: true }).order("sort_order", { ascending: true });
	if (error) throw new Error(error.message);
	return { rows: data ?? [] };
});
var slideSchema = objectType({
	...
	eyebrow: stringType().max(200).nullable().optional(),
	title: stringType().min(1).max(200),
	title_accent: stringType().max(200).nullable().optional(),
	subtitle: stringType().max(500).nullable().optional(),
	image_key: stringType().max(120).nullable().optional(),
	image_url: stringType().max(2e3).nullable().optional(),
	cta_label: stringType().max(60).nullable().optional(),
	cta_href: stringType().min(1).max(300).default("/shop"),
	align: enumType(["left", "center"]).default("left"),
	sort_order: numberType().int().default(0),
...
var upsertSlide = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => slideSchema.parse(d)).handler(upsertSlide_createServerFn_handler, async ({ data, context }) => {
	...
	const { id, ...rest } = data;
	if (id) {
		const { error } = await supabaseAdmin.from("hero_slides").update(rest).eq("id", id);
		if (error) throw new Error(error.message);
		return { ok: true };
	}
	const { error } = await supabaseAdmin.from("hero_slides").insert(rest);
	if (error) throw new Error(error.message);
	return { ok: true };
});
...
var listAdminReviews = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => d).handler(listAdminReviews_createServerFn_handler, async ({ data, context }) => {
	await assertAdmin(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	let q = supabaseAdmin.from("reviews").select("*").order("created_at", { ascending: false }).limit(500);
