...
//#region node_modules/.nitro/vite/services/ssr/assets/coupons.functions-cjzgtsBZ.js
var validateSchema = objectType({
	code: stringType().trim().min(1).max(60),
	subtotal_paise: numberType().int().nonnegative()
});
async function validateCoupon(code, subtotal_paise) {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data, error } = await supabaseAdmin.from("coupons").select("id,code,discount_type,discount_value,min_order_paise,max_discount_paise,usage_limit,usage_count,starts_at,expires_at,active,description").ilike("code", code).maybeSingle();
...
var validateCouponFn = createServerFn({ method: "POST" }).inputValidator((d) => validateSchema.parse(d)).handler(validateCouponFn_createServerFn_handler, async ({ data }) => validateCoupon(data.code, data.subtotal_paise));
