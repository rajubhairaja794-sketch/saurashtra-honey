...
//#region node_modules/.nitro/vite/services/ssr/assets/admin-cms.functions-ChjgmjSy.js
...
async function audit(actor, action, entity_type, entity_id, metadata = {}) {
...
async function seedDefaultCategoriesIfEmpty(supabaseAdmin) {
	try {
		...
		await supabaseAdmin.from("products").update({ category: "Beauty Products" }).in("name", ["Soft Skin Gel", "Royal Honey Glow Serum"]);
		await supabaseAdmin.from("products").update({ category: "Beeswax Products" }).in("category", [
			"Lip Care",
			"Body Care",
			"Wood & Leather Care"
		]);
		await supabaseAdmin.from("products").update({ category: "Honey" }).in("category", [
			"Skin Care",
			"Hair Care",
			"Single Flora"
		]);
		await supabaseAdmin.from("categories").delete().in("slug", [
			"body-care",
			"hair-care",
			"lip-care",
			"skin-care",
...
var upsertCategory = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => catSchema.parse(d)).handler(upsertCategory_createServerFn_handler, async ({ data, context }) => {
	...
	if (!id) {
		const { data: existing } = await supabaseAdmin.from("categories").select("id").eq("slug", cleanSlug).maybeSingle();
		if (existing) throw new Error(`A category with slug "${cleanSlug}" already exists.`);
	} else {
		const { data: existing } = await supabaseAdmin.from("categories").select("id").eq("slug", cleanSlug).neq("id", id).maybeSingle();
		if (existing) throw new Error(`Another category with slug "${cleanSlug}" already exists.`);
	}
	let oldName = null;
	if (id) {
		const { data: oldCat } = await supabaseAdmin.from("categories").select("name").eq("id", id).maybeSingle();
	...
	const { error } = await (id ? supabaseAdmin.from("categories").update(payload).eq("id", id) : supabaseAdmin.from("categories").insert(payload));
	if (error) throw new Error(error.message);
	if (id && oldName && oldName.trim() !== data.name.trim()) await supabaseAdmin.from("products").update({ category: data.name.trim() }).ilike("category", oldName.trim());
	await audit(context.userId, id ? "category.update" : "category.create", "category", id, { slug: cleanSlug });
	return { ok: true };
});
...
var deleteCategory = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ id: stringType().uuid() }).parse(d)).handler(deleteCategory_createServerFn_handler, async ({ data, context }) => {
	await assertPerm(context.supabase, context.userId, "categories.manage");
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: cat } = await supabaseAdmin.from("categories").select("name, slug").eq("id", data.id).maybeSingle();
	if (cat) {
		const { data: assigned, error: prodErr } = await supabaseAdmin.from("products").select("id").ilike("category", cat.name.trim()).limit(1);
...
var uploadCategoryImage = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const up = await supabaseAdmin.storage.from("media").upload(path, buf, {
	...
	await audit(context.userId, "category.image_upload", "category", void 0, { filename: data.filename });
...
var uploadProductImage = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { data: pub } = supabaseAdmin.storage.from("media").getPublicUrl(path);
	await audit(context.userId, "product.image_upload", "product", void 0, { filename: data.filename });
	return { url: pub?.publicUrl ?? null };
});
...
var upsertCoupon = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => couponSchema.parse(d)).handler(upsertCoupon_createServerFn_handler, async ({ data, context }) => {
	...
	const { error } = await (id ? supabaseAdmin.from("coupons").update(rest).eq("id", id) : supabaseAdmin.from("coupons").insert(payload));
	if (error) throw new Error(error.message);
	await audit(context.userId, id ? "coupon.update" : "coupon.create", "coupon", id, { code: data.code });
	return { ok: true };
});
...
var listPosts = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(listPosts_createServerFn_handler, async ({ context }) => {
	await assertStaff(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data, error } = await supabaseAdmin.from("blog_posts").select("*").is("deleted_at", null).order("updated_at", { ascending: false });
	if (error) throw new Error(error.message);
	return { rows: data ?? [] };
});
...
var upsertPost = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => postSchema.parse(d)).handler(upsertPost_createServerFn_handler, async ({ data, context }) => {
	...
	const { error } = await (id ? supabaseAdmin.from("blog_posts").update(rest).eq("id", id) : supabaseAdmin.from("blog_posts").insert(payload));
	if (error) throw new Error(error.message);
...
var deletePost = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ id: stringType().uuid() }).parse(d)).handler(deletePost_createServerFn_handler, async ({ data, context }) => {
	await assertPerm(context.supabase, context.userId, "blog.manage");
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { error } = await supabaseAdmin.from("blog_posts").update({ deleted_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", data.id);
	if (error) throw new Error(error.message);
...
var listCustomers = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ q: stringType().max(200).optional() }).parse(d)).handler(listCustomers_createServerFn_handler, async ({ data, context }) => {
	await assertStaff(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	let q = supabaseAdmin.from("profiles").select("*").order("created_at", { ascending: false }).limit(500);
	...
	let counts = {};
	if (ids.length) {
		const { data: orders } = await supabaseAdmin.from("orders").select("user_id,total_paise,status").in("user_id", ids).is("deleted_at", null);
...
var listMedia = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	await assertStaff(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	let q = supabaseAdmin.from("media_library").select("*").order("created_at", { ascending: false }).limit(500);
	...
	return { rows: await Promise.all((rows ?? []).map(async (r) => {
		const { data: signed } = await supabaseAdmin.storage.from(r.bucket).createSignedUrl(r.path, 3600);
		return {
			...r,
...
var deleteMedia = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ id: stringType().uuid() }).parse(d)).handler(deleteMedia_createServerFn_handler, async ({ data, context }) => {
	...
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: row, error: e0 } = await supabaseAdmin.from("media_library").select("bucket,path").eq("id", data.id).single();
...
var uploadMedia = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const up = await supabaseAdmin.storage.from(data.bucket).upload(path, buf, {
		contentType: data.contentType,
		upsert: false
	});
	if (up.error) throw new Error(up.error.message);
	const { data: row, error } = await supabaseAdmin.from("media_library").insert({
		bucket: data.bucket,
		path,
		filename: data.filename,
		mime_type: data.contentType,
...
var inviteUser = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	if (error) throw new Error(error.message);
	if (data.role && inv.user) await supabaseAdmin.from("user_roles").insert({
		user_id: inv.user.id,
		role: data.role
	});
...
var updateOrderExtended = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { data: cur } = await supabaseAdmin.from("orders").select("timeline").eq("id", data.id).single();
...
var adjustStock = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	await assertPerm(context.supabase, context.userId, "products.manage");
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: p, error: e0 } = await supabaseAdmin.from("products").select("stock_quantity").eq("id", data.product_id).single();
	if (e0) throw new Error(e0.message);
	...
	await audit(context.userId, "inventory.adjust", "product", data.product_id, { change: data.change });
	return {
		ok: true,
		before,
		after
...
var listHomepageVideos = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(listHomepageVideos_createServerFn_handler, async ({ context }) => {
	await assertStaff(context.supabase, context.userId);
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data, error } = await supabaseAdmin.from("homepage_videos").select("*").order("display_order", { ascending: true }).order("created_at", { ascending: true });
	if (error) throw new Error(error.message);
	return { rows: data ?? [] };
});
...
var upsertHomepageVideo = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => homepageVideoSchema.parse(d)).handler(upsertHomepageVideo_createServerFn_handler, async ({ data, context }) => {
	...
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { id, ...rest } = data;
	const { error } = await (id ? supabaseAdmin.from("homepage_videos").update(rest).eq("id", id) : supabaseAdmin.from("homepage_videos").insert(rest));
	if (error) throw new Error(error.message);
	await audit(context.userId, id ? "homepage_video.update" : "homepage_video.create", "homepage_video", id, { title: data.title });
	return { ok: true };
});
...
var reorderHomepageVideos = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ items: arrayType(objectType({
	...
	for (const item of data.items) {
		const { error } = await supabaseAdmin.from("homepage_videos").update({ display_order: item.display_order }).eq("id", item.id);
		if (error) throw new Error(error.message);
	}
	await audit(context.userId, "homepage_video.reorder", "homepage_video", void 0, { count: data.items.length });
	return { ok: true };
});
//#endregion
