...
//#region node_modules/.nitro/vite/services/ssr/assets/supply-services-catalog-BUS8v1Ig.js
...
async function seedWhoWeSupplyIfEmpty(supabaseAdmin) {
	try {
		const { count, error } = await supabaseAdmin.from("who_we_supply_services").select("id", {
			count: "exact",
			head: true
		});
		if (!error && (count === 0 || count === null)) {
			const rowsToInsert = DEFAULT_SUPPLY_SERVICES.map(({ id, ...rest }) => rest);
			await supabaseAdmin.from("who_we_supply_services").insert(rowsToInsert);
		}
	} catch {}
}
...
var listPublicSupplyServices = createServerFn({ method: "POST" }).handler(listPublicSupplyServices_createServerFn_handler, async () => {
	try {
		...
		const { data, error } = await supabaseAdmin.from("who_we_supply_services").select("*").eq("is_active", true).order("sort_order", { ascending: true });
...
var listAdminSupplyServices = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(listAdminSupplyServices_createServerFn_handler, async ({ context }) => {
	...
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	await seedWhoWeSupplyIfEmpty(supabaseAdmin);
	const { data, error } = await supabaseAdmin.from("who_we_supply_services").select("*").order("sort_order", { ascending: true });
	if (error) {
...
var upsertSupplyService = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => supplyServiceSchema.parse(d)).handler(upsertSupplyService_createServerFn_handler, async ({ data, context }) => {
	...
	const { id, ...rest } = data;
	if (id) {
		const { error } = await supabaseAdmin.from("who_we_supply_services").update(rest).eq("id", id);
		if (error) throw new Error(error.message);
		return { ok: true };
	}
	const { error } = await supabaseAdmin.from("who_we_supply_services").insert(rest);
	if (error) throw new Error(error.message);
	return { ok: true };
});
