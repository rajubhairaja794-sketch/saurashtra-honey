...
//#region node_modules/.nitro/vite/services/ssr/assets/orders.functions-z5auijMK.js
...
async function resolveCoupon(code, subtotalPaise) {
	if (!code) return {
		discount: 0,
		freeShipping: false,
		id: null,
	...
	const { data } = await supabaseAdmin.from("coupons").select("id,code,discount_type,discount_value,min_order_paise,max_discount_paise,usage_limit,usage_count,starts_at,expires_at,active").ilike("code", code).maybeSingle();
	if (!data) return {
		discount: 0,
		freeShipping: false,
		id: null,
...
async function insertOrderRow(data, userId) {
	...
	const { data: row, error } = await supabaseAdmin.from("orders").insert(insert).select("id, order_number").single();
...
var createOrder = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => createSchema.parse(d)).handler(createOrder_createServerFn_handler, async ({ data, context }) => insertOrderRow(data, context.userId));
...
var createGuestOrder = createServerFn({ method: "POST" }).inputValidator((d) => createSchema.parse(d)).handler(createGuestOrder_createServerFn_handler, async ({ data }) => insertOrderRow(data, null));
...
async function verifyAndMarkPaid(data, callerUserId) {
...
var verifyRazorpay = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => verifySchema.parse(d)).handler(verifyRazorpay_createServerFn_handler, async ({ data, context }) => verifyAndMarkPaid(data, context.userId));
...
var verifyRazorpayGuest = createServerFn({ method: "POST" }).inputValidator((d) => verifySchema.parse(d)).handler(verifyRazorpayGuest_createServerFn_handler, async ({ data }) => verifyAndMarkPaid(data, null));
//#endregion
