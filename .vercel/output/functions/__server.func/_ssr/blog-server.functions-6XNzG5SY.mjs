...
//#region node_modules/.nitro/vite/services/ssr/assets/blog-server.functions-6XNzG5SY.js
...
var listPublicPosts = createServerFn({ method: "POST" }).inputValidator((d) => listInputSchema.parse(d ?? {})).handler(listPublicPosts_createServerFn_handler, async ({ data }) => {
	...
	let query = supabaseAdmin.from("blog_posts").select("*", { count: "exact" }).eq("status", "published").is("deleted_at", null);
	if (data.cat && data.cat !== "All Posts") query = query.ilike("category_name", `%${data.cat.trim()}%`);
	if (data.q && data.q.trim()) {
		const term = data.q.trim().replace(/%/g, "");
		query = query.or(`title.ilike.%${term}%,excerpt.ilike.%${term}%,body_markdown.ilike.%${term}%,category_name.ilike.%${term}%`);
	}
...
var getPublicPost = createServerFn({ method: "POST" }).inputValidator((d) => objectType({ slug: stringType().min(1) }).parse(d)).handler(getPublicPost_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	const { data: post, error } = await supabaseAdmin.from("blog_posts").select("*").eq("slug", data.slug).eq("status", "published").is("deleted_at", null).maybeSingle();
	if (error || !post) return {
		post: null,
		related: []
	};
	const currentPost = post;
	const { data: relatedRows } = await supabaseAdmin.from("blog_posts").select("*").eq("status", "published").is("deleted_at", null).neq("slug", currentPost.slug).order("published_at", { ascending: false }).limit(6);
...
var getPopularPosts = createServerFn({ method: "POST" }).inputValidator((d) => objectType({ limit: numberType().int().min(1).max(20).default(4) }).parse(d ?? {})).handler(getPopularPosts_createServerFn_handler, async ({ data }) => {
	...
	const { data: popularRows } = await supabaseAdmin.from("blog_posts").select("*").eq("status", "published").is("deleted_at", null).eq("is_popular", true).order("published_at", { ascending: false }).limit(limit);
	let rows = popularRows ?? [];
	if (rows.length < limit) {
		const existingIds = rows.map((r) => r.id);
		const { data: fallbackRows } = await supabaseAdmin.from("blog_posts").select("*").eq("status", "published").is("deleted_at", null).order("published_at", { ascending: false }).limit(limit - rows.length);
		if (fallbackRows) {
...
var getFeaturedPost = createServerFn({ method: "POST" }).handler(getFeaturedPost_createServerFn_handler, async () => {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
	let { data: post } = await supabaseAdmin.from("blog_posts").select("*").eq("status", "published").is("deleted_at", null).eq("is_featured", true).order("published_at", { ascending: false }).limit(1).maybeSingle();
	if (!post) {
		const { data: latest } = await supabaseAdmin.from("blog_posts").select("*").eq("status", "published").is("deleted_at", null).order("published_at", { ascending: false }).limit(1).maybeSingle();
		post = latest;
	}
