import { a as products } from "./products-BzIDWMnY.mjs";
import { t as supabase } from "./client-CWNNtKRC.mjs";
import { r as resolveImage } from "./product-images-CqBKNuR8.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/product-catalog-nTITKWF8.js
...
function toProduct(r, varMap) {
	...
	const galleryImages = rawImages.length > 0 ? Array.from(new Set(rawImages)).slice(0, 9) : staticMatch?.images;
	const primaryImg = resolveImage(r.image_key, r.image_url, galleryImages && galleryImages.length > 0 ? galleryImages[0] : fallbackImage(r.slug));
	...
	const additionalImages = rawAdditional.length > 0 ? Array.from(new Set(rawAdditional)).slice(0, 8) : staticMatch?.additionalImages;
	return {
		slug: r.slug,
		name: r.name,
		tagline: r.tagline ?? "",
...
async function fetchProducts() {
	try {
		const { data, error } = await supabase.from("products").select("id,slug,name,tagline,description,category,flora,badge,price,price_max,mrp,rating,reviews_count,sizes,benefits,image_key,image_url,images,attributes").eq("published", true).order("sort_order", { ascending: true });
		if (error) {
...
async function fetchProduct(rawSlug) {
	...
	try {
		const { data, error } = await supabase.from("products").select("id,slug,name,tagline,description,category,flora,badge,price,price_max,mrp,rating,reviews_count,sizes,benefits,image_key,image_url,images,attributes").eq("slug", slug).maybeSingle();
		if (error) {
